...
export default function App() {
  ...
  const generatePrompt = async () => { // This is an asynchronous function that will be called when the user clicks the "Generate Prompt" button. It handles the logic for validating input, making the API request to generate the prompt, and updating the state based on the response or any errors that occur.
    ...
    3. "rhyme_scheme": A description of the rhyme scheme used. also the pattern of syllables per line (e.g., AABB with 8 syllables per line).
    4. "sample_lyrics": I want a lyric skeleton without the actual lyrics that matches the identified genre, structure, and rhyme scheme. Instead of lyrics Write the syllables per line of what the lyrics would contain (Example 8 syllables). MUST include proper Suno.ai style metatags in brackets (e.g., [Intro], [Verse 1], [Chorus], [Drop], [Guitar Solo], [Outro]). At the beginning of each section after the section tag write the rhyme scheme for that section. followed by a newline (\\n) Feel free to insert things like instrumental solos, intro music styles, etc that would fit this kind of song. Consider this will be between a 3 and 5 minute song. FORMATTING RULE: You MUST use newline characters (\\n) to separate each line of the lyrics, and use double newlines (\\n\\n) to separate different song sections. There must be a newline (\\n) right after each section tag.
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 sticky top-24">
            <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 shadow-xl backdrop-blur-sm">
              ...
              <div className="mb-6 space-y-2 border-t border-slate-800 pt-6">
                <label className="text-sm font-medium text-slate-300 flex items-center gap-2">
                  <Key className="w-4 h-4 text-yellow-400" /> Custom API Key <span className="text-xs text-slate-500">(Required for local VS Code)</span>
                </label>
                <input
                  type="password"
                  value={customKey}
              ...
              >
                {loading ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" /> Analyzing & Writing...
                  </>
                ) : (
                  <>
          ...
          <div className="lg:col-span-7">
            {result ? (
              ...
              <div className="h-full min-h-[500px] flex flex-col items-center justify-center border-2 border-dashed border-slate-800/60 rounded-2xl p-12 text-slate-500 text-center bg-slate-900/20">
                {loading ? (
                  <div className="flex flex-col items-center animate-in fade-in duration-300">
                    <Loader2 className="w-12 h-12 animate-spin text-purple-500 mb-6" />
                    <p className="text-lg text-slate-300 font-medium">Analyzing Musical Style...</p>
                    <p className="text-sm mt-2 max-w-sm">Generating Suno.ai tags, song structure, and writing original lyrics.</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-500">
                    <Sparkles className="w-12 h-12 text-slate-700 mb-6" />
                    <p className="text-lg text-slate-400 font-medium">No results generated yet.</p>
                    <p className="text-sm mt-2 max-w-sm">Fill out the form on the left to create a highly optimized Suno.ai prompt based on any artist or theme.</p>
                  </div>
                )}
              </div>
            )}
